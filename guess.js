...
alert(`Welcome to "GUESS THAT NUMBER!" Please clock "OK" to start the game.`);

// Game restarts as long as restartGame has value of true
while (restartGame){
    // Asks user to enter a number to set the upper bound for the random numer (AKA Number to be guessed).
    ...
    // Asks user to enter a quess in the range that they set.
    guess = prompt(`Please enter a guess from 1 to ${rangeNum}. You have ${attempts} attempt(s) left:`)

    // Continues looping until the user guesses the correct number or runs out of attempts (NOTE: Loops un til BREAK keyword is run)
    while (true){
        ...
        // Checks if the user guessed correctly. If so, the game ends. (NOTE: The BREAK end the loop)
        if (guess === randomNum){
    ...
    while (true){
        // Checks if the user's response is no (AKA. "N")
